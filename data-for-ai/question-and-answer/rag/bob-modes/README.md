...
- **[`base-mode/rag-builder.yaml`](base-mode/rag-builder.yaml)**: Bob mode configuration for RAG development
...
The custom Bob mode ([`base-mode/rag-builder.yaml`](base-mode/rag-builder.yaml)) defines the behavior, expertise, and capabilities of IBM Bob when working with RAG pipeline tasks.
...
```powershell
Copy-Item base-mode/rag-builder.yaml "$env:APPDATA\IBM Bob\User\globalStorage\ibm.bob-code\modes\"
```

**Linux / macOS**

```bash
cp base-mode/rag-builder.yaml ~/.config/IBM\ Bob/User/globalStorage/ibm.bob-code/modes/
```

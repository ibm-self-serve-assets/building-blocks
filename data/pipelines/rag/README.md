...
| Need a focused standalone OpenSearch ingestion service (COS → embed → index) | [`assets/opensearch-data-ingestion/`](assets/opensearch-data-ingestion/) |
...
- **OpenSearch Builder** — use when designing or tuning OpenSearch k-NN indexes and hybrid search
...
### 4. OpenSearch Data Ingestion Service
**Location**: [`assets/opensearch-data-ingestion/`](assets/opensearch-data-ingestion/)
**IBM Products**: IBM watsonx.data (OpenSearch), IBM watsonx.ai, IBM COS, IBM Cloud IAM
**Description**: Standalone FastAPI service that downloads documents from IBM Cloud Object Storage, generates IBM watsonx.ai dense embeddings, creates k-NN HNSW vector indexes in IBM watsonx.data OpenSearch, and bulk-inserts document vectors. Use this asset when you want a focused ingestion service independent of the full RAG Accelerator.

**Quick Start**:
```bash
cd assets/opensearch-data-ingestion
cp .env.example .env
# Edit .env: IBM_API_KEY, WATSONX_PROJECT_ID, OPENSEARCH_HOST, OPENSEARCH_USERNAME,
#            OPENSEARCH_PASSWORD, COS_ENDPOINT, COS_BUCKET_NAME
pip install -r requirements.txt
python main.py
# Swagger UI → http://localhost:8080/docs
```
...
| Method | Path | Description |
|---|---|---|
| `POST` | `/ingest` | Download from COS, generate embeddings, bulk-index to OpenSearch |
| `GET`  | `/` | Health check and service info |

---

### 5. RAG Retrieval FastAPI Server
...
- **[`bob-modes/`](./bob-modes/)**: Four focused AI modes for RAG and OpenSearch development
  ...
  - **OpenSearch Builder** — [`bob-modes/base-modes/opensearch-builder.zip`](./bob-modes/base-modes/opensearch-builder.zip): OpenSearch k-NN index design, HNSW parameter tuning, hybrid search (vector + BM25), watsonx.ai embedding integration
